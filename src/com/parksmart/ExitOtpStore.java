...
public class ExitOtpStore {
    ...
    public static void put(Connection conn, String slotId, String otp) {
        ...
        if (conn == null) return;
        try {
            // Delete existing rows for this slot to avoid duplicates if primary key is missing
            try (PreparedStatement del = conn.prepareStatement("DELETE FROM exit_otps WHERE slot_id = ?")) {
                del.setString(1, slotId.toUpperCase());
                del.executeUpdate();
            }
            String sql = "INSERT INTO exit_otps (slot_id, otp) VALUES (?, ?)";
